...
from config.constants import db as db_session # Use the initialized session from constants
...
def create_manager():
    ...
    try:
        ...
        if created_user:
            ...
            # If you need to create a corresponding WorkPlace entry here:
            # from db.controllers.workPlaces_controller import WorkPlacesController
            # work_places_controller = WorkPlacesController(db_session)
            # workplace_data = {
            # 'id': created_user.id, # Employee's user ID (in this case, the manager themselves)
            # 'workPlaceID': created_user.id # Manager's User ID representing the workplace
            # }
            # work_places_controller.create_entity(workplace_data)
            # print(f"WorkPlace entry created for manager ID {created_user.id}.")
...
if __name__ == "__main__":
    ...
    if not db_session:
        ...
        temp_db, _ = initialize_database_and_session()
        if temp_db:
